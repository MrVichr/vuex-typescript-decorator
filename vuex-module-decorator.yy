      ...
      get: function(this: VuextsMixin<any>) //called by user, thinking s/he calls his own method
         {
          ...
          //console.log('getter_innerm/g("+propertyKey+"): this=', this, "vi=", vi, "key=", key);
    ...
    value: function(this: VuextsMixin<any>, /*context: null,*/ payload: any) //called by user, thinking s/he calls his own method
       {
        ...
        let vi=this[$vuexts];
        vi.store_.dispatch(vi.namespacedKey(propertyKey), payload);
       }}
 }
    ...
    value: function(this: VuextsMixin<any>, payload: any) //called by user, thinking s/he calls his own method
       {
        ...
        let vi=this[$vuexts];
        let result: any=vi.store_.commit(vi.namespacedKey(propertyKey), payload);
        //result==undefined :-(
        return result;
       }};
 }
...
class VuextsInfo<T>
 {
  store_: Vuex.Store<any>; //"store" Vuex.Store<T> for store, "root" Vuex.Store<any> for module
  module_: T;
    ...
    constructor (...args:any[])
     {
      ...
      if (smi)
       {
        adjustedOptions=adjustOptionsModule<T>(that, {...adjustedOptions}, smi);;
        for (let hid of smi.hiddens)
         {
          //already initialized
          if (hid.descriptor===undefined)
...
function adjustOptionsStoMod<S>(that: S, opts: Vuex.StoreOptions<S>|Vuex.Module<S, any>, smi: StoModInfo): void
 {
  let getters: any={};
